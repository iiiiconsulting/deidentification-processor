...
def target_info(name):
    ...
    try:
        ...
        click.echo(f"Target '{name}' not found.", err=True)
        sys.exit(1)

    click.echo(f"Name:           {config['name']}")
    click.echo(f"Created:        {config.get('created', '?')}")
    click.echo(f"Spreadsheet ID: {config.get('spreadsheet_id', '?')}")
    click.echo(f"Salt:           {config['salt'][:8]}... (truncated)")
...
def process(target_name, files):
    ...
    try:
        ...
        click.echo(f"Target '{target_name}' not found. Create it first with 'deid target create'.", err=True)
        sys.exit(1)
    ...
    for sheet_type, fpaths in typed_files.items():
        ...
        deidentified, reiden_entries = deidentify(combined, sheet_type, salt)
        write_to_sheet(spreadsheet_id, sheet_type, deidentified, reiden_entries)
